...
public class GetTerrainDB {

	private final static String TERRAIN_PHP = "terrain.php";
	private static GetTerrainDB terrain = null;

	private GetTerrainDB() {
	}

	public static final GetTerrainDB getInstance() {
		return (terrain == null) ? (terrain = new GetTerrainDB()) : terrain;
	}

	public String getTerrain(String priKey) {
		priKey = priKey.replace("지형", "").replace("상성", "").trim();
		...
		String jsonstr = Communicate.toServer(Communicate.SERVER_URL + TERRAIN_PHP, s);
