...
public class ConstraintDocBuilder {
    ...
    ConstraintDocBuilder(Realm realm, AppCompatTextView summaryView, ConversationFilterObject filterObject) {
        this.realm = realm;
    ...
    public void build() {
        ...
        String[] cs_conversations = filterObject.getConversations();
        ...
        if(cs_conversations != null) {
            if(cs_conversations.length>0 && filterObject.isConvChecked()) {
                stringBuilder.append("★단어 포함:").append("\n");
                for(String cs_conversation : cs_conversations) {
                    stringBuilder.append("  - ").append(cs_conversation).append("\n");
                }
                stringBuilder.append("\n");
            }
